...
public interface UserMapper {
    ...
    @Insert("insert into public.user (name,password) values (#{name},#{password})")
...
//   用户名密码
    @Select("select name,password from public.user where name = #{name} and password = #{password}")
    User selectLogin(@Param("name")String name,@Param("password")String password);

//    检查用户名是否存在
    @Select("select count(id) from public.user where name =#{name}")
    int checkUserName(@Param("name")String name);


//    检查token
    @Select("select count(id) from public.user where token =#{token}")
